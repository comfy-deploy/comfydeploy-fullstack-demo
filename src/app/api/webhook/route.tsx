...
import { z } from "zod"; // o import * as z from "zod";
  ...
  try {
    ...
    if (!parseData.success) {
      ...
      return NextResponse.json(
        { message: "Error in webhook data", details: parseData.error.issues },
        { status: 400 }
      );
    }
    ...
    if (!run_id) {
      ...
      return NextResponse.json(
        { message: "ignored due to missing run_id" },
        { status: 200 }
      );
    }
    ...
    if (status === "queued" || status === "started" || status === "uploading") {
      console.log(
        `Status is ${status}, no outputs or image data yet, waiting for more data for run_id ${run_id}.`
      );
      return NextResponse.json(
        { message: `Status is ${status}, no outputs yet` },
        { status: 200 }
      );
    }
    ...
    if (status === "success" && outputs && outputs.length > 0) {
      ...
      if (imageData && typeof imageData === "object" && "url" in imageData) {
        ...
        console.warn(
          "No valid image data found in outputs for run_id:",
          run_id
        );
      }
    } else {
      console.log(
        "Status is not 'success' or no outputs found, skipping database update."
      );
    }
